...
import com.xing.library.helper.network.NetInterceptor
...
object NetMgr {
    ...
    fun getRetrofit(baseUrl: String, provider: NetProvider? = null): Retrofit {
        ...
        if (empty(baseUrl)) {
            throw IllegalStateException("baseUrl can not be null") as Throwable
        }
    ...
    private fun checkProvider(provider: NetProvider?) {
        if (provider == null) {
            throw IllegalStateException("NetMgr=============must register provider first")
        }
    }
